...
#normalize expression set by quantiles
meta_ml_norm <- as.data.frame(normalize.quantiles(as.matrix(meta_ml), copy=FALSE))

#create separate expression sets for normalized data
meta_ml_norm_up <- meta_ml_norm[rownames(meta_ml_norm) %in% up_genes, ]
meta_ml_norm_down <- meta_ml_norm[rownames(meta_ml_norm) %in% down_genes, ]
```

```{r}
#clustering of upregulated genes (nnormalized)
heatmap.2(as.matrix(meta_ml_norm_up), scale = "row", col = redblackgreen, trace="none", density.info = "none", distfun = function(x) dist(x,method = 'maximum'))
```

```{r}
#clustering of downregulated genes (normalized)
heatmap.2(as.matrix(meta_ml_norm_down), scale = "row", col = redblackgreen, trace="none", density.info = "none", distfun = function(x) dist(x,method = 'maximum'))
```

```{r}
#perform hierarchical clustering on normalized expression set for upregulated genes
hc_up <- hclust(dist(meta_ml_norm_up, "maximum"))
plot(hc_up)
```
...
```{r}
#perform hierarchical clustering on normalized expression set for downregulated genes
hc_down <- hclust(dist(meta_ml_norm_down, "maximum"))
plot(hc_down)
```
